...
void getAv(struct rule r[], int n_rules, int patients[][35], int n_patients, int class);
void tourEst(struct rule r[], int n_rules,  int p_index[], int n_sons, int tour_size);
void crossOver(struct rule r[], struct rule r_sons[], int p_index[], int n_sons, double mutate_percent);
...
void crossOver(struct rule r[], struct rule r_sons[], int p_index[], int n_sons, double mutate_percent){
  ...
  for (size_t i = 0; i < n_sons-1; i+=2) {
    ...
    for (size_t m = 0; m < n_points; m++) {
      struct rule aux;
      aux = r_sons[i];
      memcpy(r_sons[i].weights, r_sons[i+1].weights, sizeof(double) *points[m]);
      memcpy(r_sons[i].att, r_sons[i+1].att, sizeof(int) *points[m]);
      memcpy(r_sons[i].operators, r_sons[i+1].operators, sizeof(int) *points[m]);
